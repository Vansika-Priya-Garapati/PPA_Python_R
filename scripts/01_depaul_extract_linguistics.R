# ===============================================================================================================================================================
# DementiaBank English PPA DePaul Corpus | 01_depaul_extract_linguistics.R
...
# Comment install.packages() code after downloading packages once
install.packages("tidyverse") # Gives pipe operator (%>%), map_dfr() to process files, and write_csv for preparing csv file
install.packages("stringr") # Search and detect text patterns
install.packages("fs") # Helps R find file paths to ensure tidyverse compatibility
install.packages("udpipe") # Reads text and labels words as nouns, verbs, adjectives (Part of speech tagging)
...
# Check the versions of all 4 packages
...
# 2. File Paths
raw_cha_dir <- "data/depaul_cha" # Show R where data is
output_dir  <- "outputs" # Show R where to store extracted features in the end

# 3. Setup the English NLP model for Part of Speech (nouns,verbs, and adjectives) tagging
dl <- udpipe_download_model(language = "english") # Download 
ud_model <- udpipe_load_model(dl$file_model) # Load
# Model stored in NLP_POS_model folder

# 4. Identify, read, and extract .cha files
cha_files <- dir_ls(raw_cha_dir, glob = "*.cha") # Make a list of all .cha files

message(paste("Found", length(cha_files), "file(s). Processing PPA features...")) # Displays how many .cha files found - need to find 2
...
output_file <- path(output_dir, "ppa_depaul_linguistic_features.csv") # file destination path
